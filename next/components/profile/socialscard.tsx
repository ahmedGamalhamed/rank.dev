    ...
    <Card className="w-full sm:w-full">
      ...
      <CardContent className="flex flex-col gap-2">
        {Object.entries(socialData).map(([key, value]) => (
          <div
            key={key}
            className="flex justify-start items-center w-full sm:w-[90%]"
          >
            {getSocialIcon(key)}
            {isEditMode ? (
              <input
                value={value}
                onChange={(e) =>
                  setSocialData({ ...socialData, [key]: e.target.value })
                }
  ...
  switch (key) {
    ...
    case 'facebook':
      return (
        <FontAwesomeIcon
          icon={faFacebook}
          className="mr-2 w-8 h-8 text-blue-500"
        />
      );
    case 'github':
      return (
        <FontAwesomeIcon
          icon={faGithub}
          className="mr-2 w-8 h-8 text-white-800"
        />
      );
    case 'linkedin':
      return (
        <FontAwesomeIcon
          icon={faLinkedin}
          className="mr-2 w-8 h-8 text-blue-800"
        />
      );
    default:
      return null;
